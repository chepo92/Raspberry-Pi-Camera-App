...
class BasicControls ( BasicNotepage ):
    def BuildPage ( self ):
        ...
        v = MyBooleanVar(False)
        self.NoFocusRadio = MyRadio(f,'None (Default)',False,v,
            self.FocusChecked,0,0,'W',tip=135)
        MyRadio(f,'Autofocus','auto',v,self.FocusChecked,0,1,'W',
            tip=135)
        MyRadio(f,'Manual Focus','manual',v,self.FocusChecked,0,2,'W',
            tip=136)

        self.FocusParamText = MyStringVar("500")
        okCmdFocus = (self.register(self.ValidateFixedFocusRange),'%P')
        self.FocusParam = Entry(f,width=6,validate='all',
                validatecommand=okCmdFocus,textvariable=self.FocusParamText,
                state='disabled')
        ...
        # self.AutoFocus = MyBooleanVar(True)
        # self.AutoFocus.set(False)
        # b = ttk.Checkbutton(f,text='Enable Auto Focus',variable=self.AutoFocus,
        #     command=self.AutoFocusChecked)
        # b.grid(row=0,column=5, rowspan=1, sticky='W')
        # ToolTip(b,msg=135)
        
        
        # Label(f,text='Focus Param:').grid(row=1,column=5, rowspan=1, sticky='W')
        # #okCmd = (self.register(self.ValidateFramerateDelta),'%P')
        # self.FocusParamText = MyStringVar("0.0")
        # self.FocusParam = Entry(f,width=6,validate='all',
    ...
    def FocusChecked ( self, FocusEnabled ):
        if FocusEnabled == False:
            self.FocusParam.config(state='disabled')
            # self.focus.config(state='readonly')
            # self.FocusChanged(None)
            # self.focus.focus_set()
        elif FocusEnabled == "auto":
            print("init autofocus")
            self.set_focus()

        elif FocusEnabled == "manual":
            print("manual?")
            self.FocusParam.config(state='normal')
            print(self.FocusParamText.get())
            self.ValidateFixedFocusRange(self.FocusParamText.get())

        # else:
        #     self.FocusParam.config(state='disabled')
        #     # self.focus.config(state='disabled')
        # #     # self.ModParams.config(state='disabled')
        # #     # self.camera.image_effect = 'none'
    ...
    def ValidateFixedFocusRange ( self, EntryIfAllowed ):
        rate = self.ValidateEntry(EntryIfAllowed, 1, 1000)
        if rate != None:
            self.focusing(int(rate))
        return True
    ...
    #open camera
        ...
        self.FocusParamText.set(max_index)

        #	time.sleep(1)      
    
    # def FocusChanged ( self, event ):
    #     print("triggered focusChanged")
    #     # self.camera.image_effect = self.effects.get()
    #     # if self.camera.image_effect in ['solarize', 'colorpoint',
    #     #         'colorbalance', 'colorswap', 'posterise', 'blur', 'film',
    #     #         'watercolor']:
